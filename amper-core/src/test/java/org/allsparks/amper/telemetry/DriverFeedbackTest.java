...
class DriverFeedbackTest {
    ...
    void hysteresisHoldsElevatedUntilRecovery() {
        ...
        DriverPowerState classified = DriverFeedback.classify(
                watch, battery(12.4, 12.1), false, policy);
        ...
        DriverPowerState held = DriverFeedback.applyHysteresis(
                DriverPowerState.ELEVATED_DEMAND,
                DriverPowerState.NORMAL,
                stillLow,
                policy);
    ...
    void invalidSensingWins() {
        ElectricalObservation missing = new PowerMonitor(
                ...
                5.0,
                16.0).update();
        DriverPowerState state = DriverFeedback.classify(
                missing, null, true, PowerPolicy.defaults());
    ...
    void weakBatteryClearsWhenRecentWindowRecovers() {
        ...
        DriverPowerState weak = DriverFeedback.classify(
                loaded, battery(12.6, 10.8), false, policy);
        assertEquals(DriverPowerState.SUSPECTED_WEAK_BATTERY, weak);

        ElectricalObservation recovered = voltage(12.4);
        DriverPowerState normal = DriverFeedback.classify(
                recovered, battery(12.4, 12.4), false, policy);
